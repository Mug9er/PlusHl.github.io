...
img: https://cdn.jsdelivr.net/gh/Mug-9/imge-stroage@master/cover/wallhaven-v9z1dl.7fhsn4aycfk0.jpg
categories: [数学基础]
tags: [数学基础]
---
...
向量无穷范数：$||X||_{\infin}=\max\limits_{i\le i\le n}|x_i|$

## 不等式

### 柯西不等式

对于实数：$|\sum\limits_{i=1}^{n}a_i|\le \sum\limits_{i=1}^n|a_i|,a_i\in \R$
...
- 上极限：$\lim\limits_{n\to \infin}\sup a_n \triangleq \lim\limits_{n\to \infin}(\sup\limits_{m\ge n}a_m) = \inf\limits_{n\ge 1}(\sup\limits_{m\ge n} a_m)$
- 下极限：$\lim\limits_{n\to \infin}\inf a_n \triangleq \lim\limits_{n\to \infin}(\inf\limits_{m\ge n}a_m) = \sup\limits_{n\ge 1}(\inf\limits_{m\ge n} a_m)$
...
定义： $A \subset \R^n, \forall  x \in A$都存在以$x$为中心，$r>0$为半径的一个球$\{y\in\R^n: ||y-x|| < r\}$叫做$B(x,r)$ 然后$B(x,r)\subset A$。
...
$\{ x_k\}_{k\ge 1} \subset A, \lim\limits_{k\to \infin}x_k = x \in A$
...
若$f:D\subset \R^n \to \R$，那么$|f(x)-f(y)|\le k||x-y||$对于任何属于$D$的$x,y$都成立

若$f:D\subset \R^n \to \R$在$D$中任何点都连续，则称其在$D$上连续

- $f$在开集$D$上连续，等价于，对于任何开集$U\subset \R$，其逆像$f^{-1}(U)=\{x\in\R^n : f(x)\in U\}\subset \R^n$仍是开集
...
- $f:[a,b]\to\R$,$f$在$x\in (a,b)$取得局部最小值$\Longrightarrow f'(x)=0$
...
- $\lim\limits_{x-\to a}f(x)=\lim\limits_{x\to a}g(x)=0$或者$\lim\limits_{x\to a}g(x)=+\infin$
