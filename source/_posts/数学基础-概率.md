...
从获得的$\bold{y}$中恢复$\bold{\hat{x}}$

这里一个$y(n)$对应多个$x(n)$

从接收到的$y$中恢复$\bold{x}$, 对于同分布的高斯噪声可以看成$\bold{\hat{x}}= \mathbb{arg} \min\limits_{x}|\bold{y-Hx}|^2$

### $\bold{y=Xh+w}$

从获得的$\bold{y}$中恢复$\bold{\hat{h}}$，$\bold{X}$处于连续分布

**LS： 最小二乘法**

将$h$看成一个常量

模型$\bold{y=Sh+w}$，高斯白噪声$\bold{w}\sim N(0, \bold{I}_w)$，$\bold{h}$为未知常量

$J(\bold{h})=\bold{|y-Sh|}^2, \bold{\hat{h}} = \min\limits_{h}J(\bold{h})$

$J{(\bold{h})}=(\bold{y-Sh})^H(\bold{y-Sh})=\bold{y}^H\bold{y}-2\bold{y}^H\bold{Sh}+\bold{h}^H\bold{S}^H\bold{Sh}$

$\frac{\partial J(\bold{h})}{\partial \bold{h}}=-2\bold{S}^H\bold{y}+2\bold{S}^H\bold{Sh}$

$\bold{\hat{h}}=(\bold{S}^H\bold{S})^{-1}\bold{S}^H\bold{y}=\bold{S}^{\dagger}\bold{y}$
...
模型$\bold{y=Sh+w}$，其中$\bold{w} \sim N(0, \bold{I_w})$为高斯白噪声， 参数$h$为未知变量

令$\bold{\hat{h}} = \bold{Ay}$，已知$h$的先验信息， $\bold{h}\sim N(0, \bold{R_h})$

$A = \mathbb{arg} \min\limits_{A}E\{||\bold{h - \hat{h}}||\}=\mathbb{arg} \min\limits_{A}E\{||\bold{h} - \bold{Ay}||^2\}$

$M = E\{||\bold{h-Ay} ||^2\} = E\{tr[(\bold{h}-\bold{A}(\bold{Sh}+\bold{w}))(\bold{h}-\bold{A}(\bold{Sh}+\bold{w}))^H]\}$

$=tr\{\bold{R_h}\}-tr\{\bold{R_hS}^H\bold{A}^H\}-tr\{ \bold{ASR_h}\} + tr\{ \bold{A}(\bold{SR_h}\bold{S}^H + \sigma^2_w \bold{I}) \bold{A}^H\}$

$\frac{\partial M}{\partial \bold{A}} = 2(\bold{SR_hS}^H+\sigma_w^2\bold{I})\bold{A} - 2\bold{R_h}^H\bold{S}^H $

$\bold{A} = \bold{R_h}^H\bold{S}^H(\bold{SR_hS}^H+\sigma_w^2\bold{I})^{-1}$

$\bold{\hat{h}} = \bold{R_hS}^H(\bold{SR_hS}^H + \sigma_w^2\bold{I})^{-1}\bold{y}$
