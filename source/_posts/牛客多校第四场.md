...
$dp[2][1]$就像上图中由
 ...
 $<A,E>$,$<A,D>,$$<A,C>,$$<B,E>,$$<B,D>,$$<B,C>$ 组成，而能或出来$a$的只有$<A,E>$,那么我们就要把其余不满足的减掉，也就是减掉$dp[1][1]+dp[2][0]$,发现减多了，我们要加上$dp[1][0]+dp[0][1]$，后面就跟容斥一样，多写几个样例就会发现当$(num1+num2-i-j)$%$2$时是减,其他是加。
